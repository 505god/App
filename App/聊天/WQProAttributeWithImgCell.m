...
-(void)setDictionary:(NSDictionary *)dictionary {
    ...
    ///灰色背景
    UIView *customView = [[UIView alloc]initWithFrame:(CGRect){0,height,60,180*array.count}];
    ...
    for (int i=0; i<array.count; i++) {
        ...
        deleteBtn.frame = (CGRect){0,height+180*i+(180-60)/2,60,60};
        ...
        ///尺码
        UILabel *sizeLab = [self addLab:(CGRect){height,height+180*i+(30-20)/2,height,20}];
        ...
        UIImageView *lineView = [[UIImageView alloc]initWithFrame:(CGRect){sizeLab.right+5,height+180*i+29,[UIScreen mainScreen].bounds.size.width-sizeLab.right-10,2}];
        lineView.image = [UIImage imageNamed:@"line"];
        [self.contentView addSubview:lineView];
        
        ///价格
        UILabel *priceLab = [self addLab:(CGRect){height,height+180*i+(30-20)/2+30,height,20}];
        priceLab.text = [aDic objectForKey:@"priceTitle"];
        [self.contentView addSubview:priceLab];
        
        for (int j=0; j<4; j++) {
            WQStarView *starView0 = [[WQStarView alloc]initWithFrame:(CGRect){priceLab.right-10,priceLab.top-5+30*j,100,30} enable:NO];
            starView0.enable = NO;
            starView0.showNormal = YES;
            starView0.starNumber = j;
            [self.contentView addSubview:starView0];
            
            NSString *key = [NSString stringWithFormat:@"price%d",j];
            
            WQProductText *priceTxt0 = [self addText:(CGRect){starView0.right-10,starView0.top+5,[UIScreen mainScreen].bounds.size.width-starView0.right-10,20}];
            priceTxt0.text = [Utility checkString:[NSString stringWithFormat:@"%@",[aDic objectForKey:key]]]?[NSString stringWithFormat:@"%@",[aDic objectForKey:key]]:@"";
            priceTxt0.returnKeyType = UIReturnKeyNext;
            priceTxt0.keyboardType = UIKeyboardTypeNumbersAndPunctuation;
            priceTxt0.idxPath = self.indexPath;
            if (self.productVC) {
                priceTxt0.delegate = self.productVC;
            }else {
                priceTxt0.delegate = self.detailVC;
            }
            
            priceTxt0.tag = PriceTextFieldTag+i;
            priceTxt0.startNum = j;
            [self.contentView addSubview:priceTxt0];
            
            UIImageView *lineView20 = [[UIImageView alloc]initWithFrame:(CGRect){priceLab.right+5,starView0.bottom,[UIScreen mainScreen].bounds.size.width-priceLab.right-10,2}];
            lineView20.image = [UIImage imageNamed:@"line"];
            [self.contentView addSubview:lineView20];
        }
        
        
        ///库存
        UILabel *stockLab = [self addLab:(CGRect){height,height+180*i+(30-20)/2+30*5,height,20}];
        ...
        UIImageView *lineView3 = [[UIImageView alloc]initWithFrame:(CGRect){0,height+180*i+29+30*5,[UIScreen mainScreen].bounds.size.width,2}];
