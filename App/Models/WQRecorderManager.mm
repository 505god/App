...
- (void)startRecording {
    ...
    voiceName = [dateformat stringFromDate:[NSDate date]];
...
- (void)encapsulatingOver {
    if (self.delegate) {
        [self.delegate recordingFinishedWithFileName:filename voiceName:voiceName time:[self recordedTimeInterval]];
    }
}
...
{
    WQRecorderManager *THIS = (__bridge WQRecorderManager*)inClientData;
