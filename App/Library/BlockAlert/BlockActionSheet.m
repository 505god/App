...
- (id)initWithTitle:(NSString *)title 
{
    if ((self = [super init]))
    {
        ...
        if (title)
        {
            ...
            labelView.textColor = COLOR(160, 160, 160, 1);
...
- (void)showInView:(UIView *)view
{
    ...
    for (NSArray *block in _blocks)
    {
        ...
        if ((i-1)==_blocks.count) {
            ...
            if (Platform>=7.0) {
                UIView *lineView = [[UIView alloc]initWithFrame:(CGRect){10,_height-4,_view.bounds.size.width-kActionSheetBorder*2,0.5}];
                lineView.backgroundColor = [UIColor whiteColor];
                [_view addSubview:lineView];
                [lineView release];
                lineView = nil;
            }else {
                UIView *lineView = [[UIView alloc]initWithFrame:(CGRect){10,_height-4,_view.bounds.size.width-kActionSheetBorder*2,1}];
                lineView.backgroundColor = [UIColor whiteColor];
                [_view addSubview:lineView];
                [lineView release];
                lineView = nil;
